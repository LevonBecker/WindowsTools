...
#region Variables
	...
	# DIRECTORY PATHS
	[string]$LogPath = ($Global:WindowsToolsDefaults.GetDriveSpaceLogPath)
	...
	[string]$ResultsPath = ($Global:WindowsToolsDefaults.GetDriveSpaceResultsPath)
	...
	# FILENAMES
	[string]$ResultsTextFileName = "Get-DriveSpace_Results_" + $FileDateTime + $TimeZoneString + "_($HostInputDesc).log"
	[string]$ResultsCSVFileName = "Get-DriveSpace_Results_" + $FileDateTime + $TimeZoneString + "_($HostInputDesc).csv"
...
#region Tasks
	...
	#region Job Tasks
		...
		If ($Global:TestConnections.FailedCount -gt '0') {
			...
			[string]$FailedConnectResults = 'False,Unknown,Unknown,Unknown,False,Unknown,Unknown,Unknown,Unknown' + ',' + $Global:GetRuntime.Runtime + ',' + $ScriptStartTimeF + ' ' + $TimeZone + ',' + $Global:GetRuntime.EndTimeF + ' ' + $TimeZone + ',' + 'Failed Connection' + ',' + $ScriptVersion + ',' + $ScriptHost + ',' + $UserName
...
#					}
				...
				#region Background Job
					...
					Start-Job -ScriptBlock {
						...
						#region Setup Files and Folders
							...
							#region Temp: Rename Old Logs
								...
								If ((Test-Path -Path $OldHistoryFileFullName) -eq $true) {
									# RENAME IF NO CURRENT HISTORY LOG PRESENT
									...
									# COMBINE OLD AND NEW HISTORY LOG IF PRESENT
									Else {
										$CatLogs = Get-Content -Path $OldHistoryFileFullName
										$CatLogs += Get-Content -Path $RemoteHistoryLogFullName
										Remove-Item	-Path $OldHistoryFileFullName -Force
										Remove-Item	-Path $RemoteHistoryLogFullName -Force
										Out-File -FilePath $RemoteHistoryLogFullName -InputObject $CatLogs -Encoding ASCII -Force
									}
								}
								# REMOVE OLD LATEST LOG FILE FROM REMOTE SYSTEM
								$OldLatestFileFullName = '\\' + $ComputerName + '\c$\WindowsScriptTemp\' + $ComputerName + '_GetDiskSpace_Latest.log'
								If ((Test-Path -Path $OldLatestFileFullName) -eq $true) {
									Remove-Item	-Path $OldLatestFileFullName -Force
								}
						...
						#region Generate Report
							...
							#region Output Results to File
								...
								[string]$TaskResults = $ComputerName + ',' + $CompleteSuccess + ',' + $DiskSpaceOK + ',' + $DriveSize + ',' + $FreeSpace + ',' + $ConnectSuccess + ',' + $OSVersion + ',' + $OSArch + ',' + $HostIP + ',' + $HostDomain + ',' + $Global:GetRuntime.Runtime + ',' + $JobStartTimeF + ' ' + $TimeZone + ',' + $Global:GetRuntime.EndTimeF + ' ' + $TimeZone + ',' + $ScriptErrors + ',' + $ScriptVersion + ',' + $ScriptHost + ',' + $UserName
					...
					} -ArgumentList $ComputerName,$Assets,$SubScripts,$JobLogFullName,$ResultsTextFullName,$ScriptHost,$UserDomain,$UserName,$SubScripts,$LogPath,$ScriptVersion,$MinFreeMB,$ResultsTempPath,$WIPTempPath,$TimeZone | Out-Null
