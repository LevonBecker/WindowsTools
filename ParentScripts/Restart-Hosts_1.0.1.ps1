...
.EXAMPLE
	Restart-Hosts -FileBrowser -SkipOutGrid
	FileBrowser:
		This switch will launch a separate file browser window.
		In the window you can browse and select a text or csv file from anywhere
		accessible by the local computer that has a list of host names.
...
.LINK
	http://wiki.bonusbits.com/main/PSScript:Restart-Hosts
...
#region Variables
	...
	[string]$ScriptTitle = "Restart Hosts by Levon Becker"
	[int]$DashCount = '30'
	...
	# DIRECTORY PATHS
	[string]$LogPath = ($Global:WindowsToolsDefaults.RestartHostsLogPath)
	...
	[string]$ResultsPath = ($Global:WindowsToolsDefaults.RestartHostsResultsPath)
	...
	# FILENAMES
	[string]$ResultsTextFileName = "Restart-Hosts_Results_" + $FileDateTime + $Timezone + "_($HostInputDesc).log"
	[string]$ResultsCSVFileName = "Restart-Hosts_Results_" + $FileDateTime + $Timezone + "_($HostInputDesc).csv"
...
#region Tasks
	...
	#region Job Tasks
		...
		If ($Global:TestConnections.FailedCount -gt '0') {
			...
			[string]$FailedConnectResults = 'False,Unknown,Unknown,False,Unknown,Unknown,Unknown,Unknown,Unknown' + ',' + $Global:GetRuntime.Runtime + ',' + $ScriptStartTimeF + ',' + $Global:GetRuntime.EndTimeF + ',' + 'Failed Connection' + ',' + $ScriptVersion + ',' + $ScriptHost + ',' + $UserName
...
#			[boolean]$FirstGroup = $false
			Foreach ($ComputerName in $HostList) {
				...
				# Progress Bar
				Write-Progress -Activity "STARTING JOB ON - ($ComputerName)" -PercentComplete $TaskProgress -Status "OVERALL PROGRESS - $TaskProgress%"
				...
				#region Background Job
					...
					Start-Job -ScriptBlock {
						...
						#region Job Variables
							...
							# HISTORY LOG
							[string]$HistoryLogFileName = $ComputerName + '_RestartHosts_History.log' 
							...
							# LATEST LOG
							[string]$LatestLogFileName = $ComputerName + '_RestartHosts_Latest.log' 
						...
						#region Reboot Computer
						
							# ADD TO SCRIPT LOG ARRAY
							$DateTimeF = Get-Date -format g
							$ScriptLogData += @(
								'',
								"REBOOTING:       [$ComputerName] ($DateTimeF)"
							)
							
							Restart-Host -ComputerName $ComputerName
#							$Global:RebootCount++
							$RebootTime = $Global:RestartHost.RebootTime
						...
						#region Generate Report
							...
							#region Output Results to File
								...
								[string]$TaskResults = $ComputerName + ',' + $RebootSuccess + ',' + $RebootTime + ',' + $ConnectSuccess + ',' + $OSVersion + ',' + $OSArch + ',' + $HostIP + ',' + $HostDomain + ',' + $DriveSize + ',' + $FreeSpace + ',' + $Global:GetRuntime.Runtime + ',' + $JobStartTimeF + ',' + $Global:GetRuntime.EndTimeF + ',' + $ScriptErrors + ',' + $ScriptVersion + ',' + $ScriptHost + ',' + $UserName
	...
	#region Job Monitor
		...
		Watch-Jobs -JobLogFullName $JobLogFullName -Timeout $JobQueTimeout -Activity "TRIGGERING REBOOTS" -WinTitleInput $Global:WinTitleInput
...
#region Cleanup WIP
	...
	If ((Test-Path -Path "$WIPTempPath\*") -eq $true) {
		Get-Runtime -StartTime $ScriptStartTime
		[string]$TimedOutResults = 'False,Unknown,Unknown,True,Unknown,Unknown,Unknown,Unknown,Unknown' + ',' + $Global:GetRuntime.Runtime + ',' + $ScriptStartTimeF + ',' + $Global:GetRuntime.EndTimeF + ',' + 'Timed Out' + ',' + $ScriptVersion + ',' + $ScriptHost + ',' + $UserName
...
1.0.6 - 12/26/2012
	...
	Switched to Func_Watch-Jobs 1.0.5
	Added Get-HostDomain
	...
	Changed the Get Hard Drive Space to check for user entered Min and Log Space check.
