...
Function Get-HostDomain {
	...
	If ($global:GetHostDomain) {
		Remove-Variable GetHostDomain -Scope "Global" | Out-Null
	}
	...
	If ($ComputerName) {
		...
		#region vCenter Query
			...
			If (($wmiconnect -eq $false) -and ($SkipVimQuery.IsPresent -eq $false)) {
				# . "$SubScripts\Get-VmGuestInfo_1.0.5.ps1"
	...
	$global:GetHostDomain = New-Object -TypeName PSObject -Property @{
