...
Function Get-ExitCodeDescription {
	...
	If ($Global:GetExitCodeDescription) {
		Remove-Variable GetExitCodeDescription -Scope "Global"
	}
	...
	#region Tasks
		...
		$CrossReferenceTable = @{
			...
			1015 = "The registry is corrupted. The structure of one of the files containing registry data is corrupted or the systems memory image of the file is corrupted or the file could not be recovered because the alternate copy or log was absent or corrupted."
			1016 = "An I/O operation initiated by the registry failed unrecoverably. The registry could not read in or write out or flush one of the files that contain the systems image of the registry."
			...
			1022 = "A notify change request is being completed and the information is not being returned in the callers buffer. The caller now needs to enumerate the files to find the changes."
			...
			1081 = "This service runs in the same process as the service control manager. Therefore the service control manager cannot take action if this services process terminates unexpectedly."
			...
			1276 = "A dynamic link library (DLL) referenced a module that was neither a DLL nor the processs executable image."
			...
			1374 = "The user cannot be removed from a group because the group is currently the users primary group."
			...
			1384 = "During a logon attempt the users security context accumulated too many security IDs."
			...
			1397 = "Mutual Authentication failed. The servers password is out of date at the domain controller."
			...
			1793 = "The users account has expired."
			...
			1907 = "The users password must be changed before logging on the first time."
			...
			2021 = "The specified transform does not match the bitmaps color space."
			...
			5897 = "Two or more parameter values specified for a resources properties are in conflict."
			...
			8329 = "The operation could not be performed because the objects parent is either uninstantiated or deleted."
			...
			8483 = "Source and destination of a cross domain move do not agree on the objects epoch number. Either source or destination does not have the latest version of the object."
			8484 = "Source and destination of a cross domain move do not agree on the objects current name. Either source or destination does not have the latest version of the object."
			...
			8493 = "Cant move objects with memberships across domain boundaries as once moved this would violate the membership conditions of the account group. Remove the object from any account group memberships and retry."
			...
			8537 = "The operation couldnt locate a DC for the source domain."
			8538 = "The source object must be a group or user."
			8539 = "The source objects SID already exists in destination forest."
			...
			8601 = "The remote create cross reference operation failed on the Domain Naming Master FSMO. The operations error is in the extended data."
			...
			13828 = "Peers certificate did not have a public key."
			...
			13848 = "Certification Revocation check of peers certificate failed."
			...
			14028 = "A components file does not match the verification information present in the component manifest."
			...
			14076 = "Assembly Protection Error: The catalog for an assembly is not valid or does not match the assemblys manifest."
		...
		If (($CrossReferenceTable.ContainsKey($ExitCode)) -eq $true) {
			[string]$Description = $CrossReferenceTable[$ExitCode]
	...
	$Global:GetExitCodeDescription = New-Object -TypeName PSObject -Property @{
...
<# Description
	This script will return the Windows System Error Code Description for a particular exit code number.
#>
